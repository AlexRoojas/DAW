...
public class PeliculasJpaController implements Serializable {
    ...
    public List <Peliculas> peliculasPorGenero(String g){
        ...
        q.setParameter("genero", q);
        return q.getResultList();
    }
    
    public List <Peliculas> peliculasPorGenEst(String g,boolean e){
        EntityManager em = getEntityManager();
        
        Query q=em.createNamedQuery("Peliculas.miConsulta");
        q.setParameter("genero", q);
        q.setParameter("estreno", q);
        return q.getResultList();
    }
    ...
    public List peliculaDirector(){
        ...
        Query q=em.createQuery("select p.nombre, p.director p from Peliculas p");
        
        
        return q.getResultList();
    }
    
    public List consultaNativa(){
        EntityManager em = getEntityManager();
        
        ResultSetMappingQuery rsmq;
        
        //Query q=em.createNativeQuery("Select Distinc genero from peliculas");
        Query q=em.createNativeQuery("select nombre,director from peliculas","rsmq");
