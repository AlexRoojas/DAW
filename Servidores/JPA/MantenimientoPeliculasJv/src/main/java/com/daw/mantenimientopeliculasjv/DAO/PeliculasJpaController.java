 ...
 */
package com.daw.mantenimientopeliculasjv.DAO;
...
public class PeliculasJpaController implements Serializable {
    ...
    public List obtenerCabeceras(){
        EntityManager em = getEntityManager();
        Query q1 = em.createNativeQuery("select COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA = 'peliculas' and TABLE_NAME = 'peliculas'order by ORDINAL_POSITION");
        Query q2 = em.createNativeQuery("SELECT `COLUMN_NAME` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_NAME`='peliculas'");
        Query q3 = em.createNativeQuery("show columns from peliculas");
