...
function enviarPeticion(metodo,url,asincrono,procesaRespuesta,procesaError,datos){
    ...
    //2.- Programamos el evento de respuesta
...
//-------------------------------------------------------------------------------
function loadDocXML(fichero,asinc,procesaResp,id){
      
  //var xhttp = creaXMLHttpR();
    var xhttp = new XMLHttpRequest();
    
    xhttp.onload=function (){
        procesaResp(this.responseXML,id);
    }
...
//------------------Cargar CD's en la lista y en un Objeto-----------------------
    ...
    function Cargar(xmlDoc,id){
        ...
        //xmlDoc = loadDocXML("xml/cdcatalog.xml",false);//hasta que loadDocXML no termine no sigue para abajo(asinc)
        //xslDoc = loadDocXML("xml/cdcatalog.xsl",true);

        //cargar lista y crear objeto
        ...
        //usamos tit.length porque son las veces que se ha usado el titulo
        //es como xmlDoc.children[0].children.length
        //1.- Cargar Select
        for (var i = 0; i < tit.length; i++) {
            select.options.add(new Option(tit[i].childNodes[0].nodeValue+" / "
